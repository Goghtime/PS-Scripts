param (
    [string]$DataFolderPath = "C:\Windows\Admin\Data\Chrome\Downloads",
    [string]$ChromeHistoryLogPath = "C:\Windows\Admin\logs"
)

# Set up logging
$LogFileName = "ChromeDownloads.log"
...
function Manage-CSVFiles {
    $CsvFiles = Get-ChildItem -Path $DataFolderPath -File -Filter "ChromeDownloads_*.csv" | Sort-Object -Property LastWriteTime -Descending
...
Log-Message "Starting Chrome Download history processing."
...
Log-Message "Processed Chrome Download history for user: $UserName"
}
catch {
Log-Message "Error processing Chrome Download history for user: $UserName"
Log-Message $_.Exception.Message
}
}
else {
Log-Message "Chrome Download history not found for user: $UserName"
}
}

# Export the combined results as a single CSV file with a timestamp
$TimeStamp = Get-Date -Format 'yyyyMMddHHmmss'
$CsvFileName = "ChromeDownload_$TimeStamp.csv"
